...
A .NET Console application that reads pre-extracted text files (from PdfTextExtractor) and generates embeddings using Semantic Kernel with Ollama, LM Studio, or OpenAI.

**Note:** The Preprocessor has no secrets - all configuration is via CLI arguments (documented below).

## Purpose

Convert pre-extracted PDF text into searchable embeddings (run once, or when adding new PDFs).

![preprocessor demo](docs/preprocessor_running.gif)

## Prerequisites

**IMPORTANT:** The Preprocessor requires pre-extracted text files generated by PdfTextExtractor. You must run PdfTextExtractor first to create text files in the format `{PdfBaseName}_page_{PageNumber}.txt` before running the Preprocessor.

See [PdfTextExtractor README](../PdfTextExtractor/ReadMe.Md) for text extraction instructions.
...
Text files must:
- Follow naming pattern: `{PdfBaseName}_page_{PageNumber}.txt`
- Have sequential page numbers starting at 1 (no gaps)
- Exist in the same directory as the PDF files
...
- **Implementation:** `TextFileExtractor` - Reads pre-extracted text files from PdfTextExtractor
- **Chunking:** Delegates to `SemanticChunker` for paragraph-based semantic chunking with overlap
- **Process:** Read text files → Chunk text (preserve formatting) → Return `DocumentChunk` objects
- **Note:** Text sanitization is handled by PdfTextExtractor, not Preprocessor
...
│  ├─ Read pre-extracted text files per page
│  ├─ Preserve original formatting (no cleaning)
│  └─ ITextChunker.Chunk() [semantic paragraph-based with overlap]
