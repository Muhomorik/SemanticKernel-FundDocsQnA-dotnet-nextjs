...
public class FundListOrchestrator : IFundListOrchestrator
{
    ...
    private readonly IFundListSessionScheduler _sessionScheduler;
    ...
    public FundListOrchestrator(
        ...
        IFundListSessionScheduler sessionScheduler,
    ...
    public FundListSessionId StartSession()
    {
        ...
        _currentSessionId = _sessionScheduler.ScheduleSession(FundListSessionScheduler.DefaultExpectedBatchCount);
        _logger.Info("Session {0} scheduled with {1} batches", _currentSessionId, FundListSessionScheduler.DefaultExpectedBatchCount);

        RefreshStateFromEventStore();
        RefreshScheduledBatches();

        // Start first batch immediately
        var firstBatch = new FundListBatchNumber(1);
        _eventStore.Append(FundListBatchStarted.Create(_currentSessionId.Value, firstBatch));
    ...
    public async Task NotifyBatchLoadedAsync(IReadOnlyCollection<FundListDataDto> funds, int totalFundsLoaded, bool hasMore)
    {
        ...
        _eventStore.Append(FundListBatchCompleted.Create(
            sessionId,
            new FundListBatchNumber(completedBatchNumber),
            fundsInBatch,
            totalFundsLoaded));
    ...
    private void StartCountdown(FundListBatchScheduled scheduled)
    {
        ...
        _countdownSubscription = Observable
            ...
            .Subscribe(
                tick =>
                {
                    ...
                    _countdownTick.OnNext(new FundListCountdownTick(scheduled.BatchNumber, remaining, totalDelay));
    ...
    private FundListSessionState ProjectStateFromEventStore()
    {
        var activeSession = _eventStore.GetActiveSession();
        ...
        var lastDelayStarted = sessionEvents.OfType<FundListBatchDelayStarted>().LastOrDefault();
        var lastDelayCompleted = sessionEvents.OfType<FundListBatchDelayCompleted>().LastOrDefault();
    ...
    private void RefreshScheduledBatches()
    {
        ...
        var scheduled = sessionEvents.OfType<FundListBatchScheduled>().ToList();
        var started = sessionEvents.OfType<FundListBatchStarted>().Select(e => e.BatchNumber).ToHashSet();
        var completed = sessionEvents.OfType<FundListBatchCompleted>().ToDictionary(e => e.BatchNumber, e => e.FundsInBatch);
        var failed = sessionEvents.OfType<FundListBatchFailed>().Select(e => e.BatchNumber).ToHashSet();

        var items = scheduled.Select(s =>
        {
            FundListBatchStatus status;
            if (completed.ContainsKey(s.BatchNumber))
                status = FundListBatchStatus.Completed;
            else if (failed.Contains(s.BatchNumber))
                status = FundListBatchStatus.Failed;
            else if (started.Contains(s.BatchNumber) && !completed.ContainsKey(s.BatchNumber))
                status = FundListBatchStatus.InProgress;
            else
                status = FundListBatchStatus.Pending;

            return new FundListScheduledBatchItem
            {
