...
/// </para>
/// </remarks>
public class DualWriteFundListIngestionService : IFundListIngestionService
{
    private readonly ILogger _logger;
    private readonly IFundListIngestionService _inner;
    ...
    public DualWriteFundListIngestionService(
        ILogger logger,
        IFundListIngestionService inner,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var funds = fundDataList as IList<FundListDataDto> ?? fundDataList.ToList();
    ...
    private async Task SyncToBackendAsync(IList<FundListDataDto> funds)
    {
        try
        {
            ...
            foreach (var dto in funds)
            {
                ...
                // FundProfile does not have Nav/NavDate — using profile.ToApiFundListDto() would
                // produce null values, causing the backend to silently skip history records.
                var apiDto = dto.ToApiFundListDto();
                if (profile != null)
                {
                    apiDto = apiDto with
                    {
