...
/// <summary>
/// Extension methods for converting <see cref="FundListInterceptedFund"/> to <see cref="FundListDataDto"/>.
/// </summary>
public static class FundListDataDtoMapper
{
    /// <summary>
    /// Converts an <see cref="FundListInterceptedFund"/> to a <see cref="FundListDataDto"/>.
    /// </summary>
    ...
    public static FundListDataDto ToFundListDataDto(this FundListInterceptedFund fund)
    {
        return new FundListDataDto
        {
            // Identifiers
            Isin = fund.Isin,
            Name = fund.Name,
    ...
    /// <summary>
    /// Converts a collection of <see cref="FundListInterceptedFund"/> to <see cref="FundListDataDto"/> collection.
    /// </summary>
    /// <param name="funds">The intercepted funds.</param>
    /// <returns>A read-only collection of DTOs.</returns>
    public static IReadOnlyCollection<FundListDataDto> ToFundListDataDtos(this IEnumerable<FundListInterceptedFund> funds)
    {
        return funds.Select(f => f.ToFundListDataDto()).ToList();
    }
}